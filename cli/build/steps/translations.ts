...
function createJsonTranslationFiles(descriptor: TFolderReplicationDescriptor): void {
  ...
  const heading: string = `<?xml version=${quoted("1.0")} encoding="windows-1251" ?>\n\n`;
  ...
  LOCALES_TO_PROCESS.forEach((locale) => {
    ...
    fs.writeFileSync(targetFile, encode(renderJsxToXmlText(targetData, heading + comment), "windows-1251"));
  });
}
