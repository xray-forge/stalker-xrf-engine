...
export function setupBuildCommands(command: Command): void {
  command
    ...
    .addOption(new Option("--no-lua-logs", "strip all lua logs from target build"))
    .addOption(new Option("--no-asset-overrides", "skip step with building additional assets"))
    .action(build);
}
