...
export function renderJsxToXmlText(node: JSXNode): string {
  ...
  const autoGeneratedComment: string = `<!-- Autogenerated by xrts util at: ${new Date().toString()} -->\n\n`;

  return autoGeneratedComment + formatXml(render(node).slice(xmlMeta.length), "  ") + "\n";
}
