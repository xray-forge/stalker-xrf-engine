...
import { notificationManagerIcons } from "@/mod/scripts/core/manager/notifications";
...
import { isImmuneToSurge, isObjectOnLevel, isSurgeEnabledOnLevel } from "@/mod/scripts/utils/check/check";
import { isStoryObject } from "@/mod/scripts/utils/check/is";
import { pickSectionFromCondList } from "@/mod/scripts/utils/config";
import { executeConsoleCommand } from "@/mod/scripts/utils/console";
import { disableGameUiOnly } from "@/mod/scripts/utils/control";
import { createScenarioAutoSave, setLoadMarker, setSaveMarker } from "@/mod/scripts/utils/game_save";
...
export class SurgeManager extends AbstractCoreManager {
  ...
  protected giveSurgeHideTask(): void {
    if (this.surge_task_sect !== "empty") {
      ...
      const { get_task_manager } = require("@/mod/scripts/core/manager/tasks/TaskManager");
