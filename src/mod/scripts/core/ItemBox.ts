...
const death_ini = new ini_file("misc\\ph_box_generic.ltx");
...
export class ItemBox {
  ...
  public constructor(object: XR_game_object) {
    ...
    for (const [k, v] of pairs(community_list)) {
      ...
      if (death_ini.section_exist(v)) {
        ...
        for (const i of $range(0, n - 1)) {
          const [result, id, value] = death_ini.r_line(v, i, "", "");
    ...
    if (!death_ini.section_exist(level_name)) {
      level_name = "default";
    }

    for (const i of $range(0, death_ini.line_count(level_name) - 1)) {
    ...
    for (const i of $range(0, death_ini.line_count(item_count_section) - 1)) {
