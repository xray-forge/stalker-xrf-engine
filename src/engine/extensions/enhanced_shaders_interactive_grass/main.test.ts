...
import { register, unregister } from "@/engine/extensions/enhanced_shaders_interactive_grass/main";
...
describe("shadow cascades extension", () => {
  ...
  it("should correctly call console commands with register", () => {
    register();
  ...
  it("should correctly call console commands with unregister", () => {
    unregister();

    expect(console.execute).toHaveBeenCalledTimes(3);
    expect(console.execute).toHaveBeenCalledWith("ssfx_grass_interactive (0,0,0,0)");
    expect(console.execute).toHaveBeenCalledWith("ssfx_int_grass_params_1 (0,0,0,0)");
    expect(console.execute).toHaveBeenCalledWith("ssfx_int_grass_params_2 (0,0,0,0)");
  });
});
