...
import { register, unregister } from "@/engine/extensions/enhanced_shaders_shadow_cascades/main";
...
describe("shadow cascades extension", () => {
  ...
  it("should correctly call console commands on register", () => {
    register();
  ...
  it("should correctly call console commands on unregister", () => {
    unregister();

    expect(console.execute).toHaveBeenCalledTimes(2);
    expect(console.execute).toHaveBeenCalledWith("ssfx_shadow_cascades (0,0,0)");
    expect(console.execute).toHaveBeenCalledWith("ssfx_grass_shadows (0,0,0,0)");
  });
});
