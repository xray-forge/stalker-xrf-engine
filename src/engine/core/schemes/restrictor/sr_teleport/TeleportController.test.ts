...
describe("TeleportController", () => {
  it("should correctly call updates, teleport and react to generic logic", () => {
    ...
    const teleportController: TeleportController = getSchemeAction(schemeState);

    expect(state.activeSection).toBe("sr_teleport@test");

    jest.spyOn(Date, "now").mockImplementation(() => 20_000);
    teleportController.update();
    expect(teleportController.teleportState).toBe(ETeleportState.IDLE);

    replaceFunctionMock(object.inside, () => true);
    teleportController.update();
    expect(teleportController.teleportState).toBe(ETeleportState.ACTIVATED);
