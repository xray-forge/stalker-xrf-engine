...
function createController(base: Partial<ISchemeCombatIgnoreState> = {}): {
  controller: CombatProcessEnemyController;
  object: GameObject;
  ...
  return { controller: new CombatProcessEnemyController(object, state), object, state };
}

describe("CombatProcessEnemyController", () => {
  beforeEach(() => {
    resetRegistry();
  ...
  it("should register actor combat when actor becomes an enemy", () => {
    ...
    expect(controller.onObjectEnemy(object, actor)).toBe(true);
  ...
  it("should not select enemy that is filtered out by danger utils", () => {
    ...
    expect(controller.onObjectEnemy(object, MockGameObject.mock())).toBe(false);
  ...
  it("should not raise alarm for object outside of a smart terrain", () => {
    ...
    expect(controller.onObjectEnemy(object, MockGameObject.mock())).toBe(true);
    expect(startTerrainAlarm).not.toHaveBeenCalled();
  });
  ...
  it("should raise smart terrain alarm when object is in a terrain", () => {
    ...
    expect(controller.onObjectEnemy(object, enemy)).toBe(true);
  ...
  it("should reject enemies that are too far away", () => {
    ...
    expect(controller.onObjectEnemy(object, enemy)).toBe(false);
  });
  ...
  it("should ignore hits without source or damage", () => {
    ...
    controller.onHit(object, 0, ZERO_VECTOR, MockGameObject.mock({ id: ACTOR_ID }), 1);
    expect(state.enabled).toBe(true);
  });
  ...
  it("should disable combat ignore when actor hits the object", () => {
    ...
    controller.onHit(object, 10, ZERO_VECTOR, MockGameObject.mock({ id: ACTOR_ID }), 1);
