...
export class ActionProcessEnemy {
  public static isEnemy(object: ClientObject, enemy: ClientObject, state: ISchemeCombatIgnoreState): boolean {
    if (!object.alive()) {
      return false;
    }
   ...
   * todo: Description.
   */
  public enemy_callback(object: ClientObject, enemy: ClientObject): boolean {
    if (enemy.id() === registry.actor.id()) {
