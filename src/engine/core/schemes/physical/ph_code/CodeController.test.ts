...
describe("CodeController", () => {
  it("should correctly activate", () => {
    ...
    const controller: CodeController = new CodeController(object, state);
  ...
  it("should handle usage with no exceptions", () => {
    ...
    expect(() => controller.onUse()).not.toThrow();
  });
  ...
  it("should handle number callback when have code", () => {
    ...
    controller.onNumberReceive("12");
    expect(first).not.toHaveBeenCalled();
    expect(second).not.toHaveBeenCalled();

    controller.onNumberReceive("123");
    expect(first).not.toHaveBeenCalled();
  ...
  it("should handle number callback when have check", () => {
    ...
    controller.onNumberReceive("x12");
    expect(first).not.toHaveBeenCalled();
    expect(second).not.toHaveBeenCalled();

    controller.onNumberReceive("x123");
    expect(first).not.toHaveBeenCalled();
    expect(second).toHaveBeenCalled();

    controller.onNumberReceive("x443");
