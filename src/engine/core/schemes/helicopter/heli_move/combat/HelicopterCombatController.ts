...
/**
 * Controller handling helicopter combat behaviour, switching between fly-by, round, search and retreat tactics.
 */
export class HelicopterCombatController {
  ...
  /**
   * Save the combat controller state into the network packet.
   *
   * @param packet - Network packet to write the combat state into.
   */
  public save(packet: NetPacket): void {
    openSaveMarker(packet, HelicopterCombatController.name);

    if (isGameLevelChanging()) {
      packet.w_bool(false);
      closeSaveMarker(packet, HelicopterCombatController.name);
  ...
  /**
   * Load the combat controller state from the network reader.
   *
   * @param reader - Network reader to read the combat state from.
   */
  public load(reader: NetReader): void {
    openLoadMarker(reader, HelicopterCombatController.name);
    ...
    closeLoadMarker(reader, HelicopterCombatController.name);
  }
  ...
  /**
   * Clear the current enemy and reset the combat controller to an uninitialized state.
   */
