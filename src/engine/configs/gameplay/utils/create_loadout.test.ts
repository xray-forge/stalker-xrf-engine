...
describe("create_loadout util", () => {
  it("should correctly generate resulting strings", () => {
    expect(createLoadout([{ section: "test_sect" }])).toBe("test_sect = 1, prob = 1 \\n\r\n");
    expect(createLoadout([{ section: "test_sect", count: 4 }])).toBe("test_sect = 4, prob = 1 \\n\r\n");
    ...
    expect(
      createLoadout([
        ...
        { section: "another_sect", probability: 0.25, silencer: true },
      ])
